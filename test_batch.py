...
def main(args):
    ...
    def filt_func(obj):
        if int(obj["is_correct"]) == 1:
            return True
            # return False
        else:
            return False
            # return True
    reader = DatasetReader.from_params(params["dataset_reader"], filt_func=filt_func)
    ...
    if test_dataset:
        ...
        with open(os.path.join(args.output_dir, "test_metrics_uncorrect.txt"), "w", encoding="utf-8") as f_out:
